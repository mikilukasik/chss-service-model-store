...
const modelDirName = 'models/3sizesV1tiny';
...
const buildModel = function () {
  ...
  const conv1a = convBlock({ input, kernelSize: 8, filters: 6 });
  const conv1b = convBlock({ input: conv1a, kernelSize: 8, filters: 32 });
  const flat1 = tf.layers.flatten().apply(conv1b);

  const conv2a = convBlock({ input, kernelSize: 5, filters: 5 });
  const conv2b = convBlock({ input: conv2a, kernelSize: 5, filters: 24 });
  const flat2 = tf.layers.flatten().apply(conv2b);

  const conv3a = convBlock({ input, kernelSize: 3, filters: 4 });
  const conv3b = convBlock({ input: conv3a, kernelSize: 3, filters: 16 });
  ...
  const dense1 = tf.layers.dense({ units: 256, activation: tf.leakyReLU, useBias: false }).apply(concatenated1);
